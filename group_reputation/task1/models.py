...
class Player(BasePlayer):
    birth_region = models.IntegerField(choices=[
        [i+1, Constants.regions[i]] for i in range(len(Constants.regions))
    ], widget=widgets.RadioSelect)
    other_br = models.StringField(label='Other region', blank=True)
    reduced_birth_region = models.IntegerField()
    ...
    def find_pi(self):
        # determines the political ideology of a participant
        pi_arr = [self.pi_q1, self.pi_q2, self.pi_q3, self.pi_q4, self.pi_q5, self.pi_q6, self.pi_q7]
        pi_fraction = sum(pi_arr) / (len(pi_arr) * len(Constants.choices))
        
        if pi_fraction >= 0.7:
            self.pol_ideology = 1
        elif pi_fraction <= 0.3:
            self.pol_ideology = 2
        else:
            self.pol_ideology = 3
