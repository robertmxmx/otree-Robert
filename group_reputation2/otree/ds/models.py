...
class Player(BasePlayer):
    area_of_study = models.StringField(
        ...
        choices=['Economics', 'Engineering', 'Marketing', 'Physics', 'Accounting', 'Humanities', 'Social Sciences',
                 'Other', 'Not a student'],
        widget=widgets.RadioSelect
    )
    supporting_competition = models.IntegerField(
        label="Do you support a football team in one of the following competitions?",
        choices=[[1, 'English premier league football'], [2, 'Australian rules (AFL)'],
                 [3, 'American football (NFL)'], [4, 'None']],
        widget=widgets.RadioSelect
    )
    ...
    retaliate_type = models.LongStringField(label='''Suppose in the experiment A took 100 ECU from B, and B then 
    ...
    retaliate_beh = models.LongStringField(label='What do you think of retaliation behavior in general?')
